...
// will perform one pass and see if the footer could exist in this part of the
// buffer or later
...
// looks for the footer and returns both whether it was found and also at what
// position in the buffer it STARTS
...
std::pair<char *, size_t> get_message(int sock, bool block) {
  ...
  while (1) {
    ...
    if (ret.first) {
      ...
      // size_t actually_read = ret.second + HEADER_LENGTH + (total_got >
      // HEADER_LENGTH ? HEADER_LENGTH : total_got);  ssize_t actually_read =
      // ret.second - (total_got > HEADER_LENGTH ? HEADER_LENGTH : 0);
      ssize_t actually_read =
          ret.second - (off < total_got ? HEADER_LENGTH : 0);

      // now remove those bytes from the read queue, this should leave the next
      // header right there to read
      ...
      return std::make_pair(retbuf + HEADER_LENGTH,
                            total_got + actually_read - 2 * HEADER_LENGTH);
    }
    // else keep going
...
void free_message(char *msg) { free(msg - HEADER_LENGTH); }
...
bool send_message(int sock, char *buf, size_t num_bytes) {
  ...
  */
  ...
  while (left > 0) {
    ...
    int flags =
        (MSG_MORE); // due to header/footer we need to add more every time
    flags = 0;
    ssize_t sent =
        send(sock, buf + total_sent, std::min(left, (size_t)mtu), flags);
...
bool send_read_confirm(int sock, size_t lines, std::string filename) {
  ...
  *(size_t *)(buf + sizeof(short)) = htonll(lines);
  memcpy(buf + sizeof(short) + sizeof(size_t), filename.c_str(),
         filename.length() + 1);
...
bool broadcast_read_confirm(const std::vector<int> &sockets, size_t lines,
                            std::string filename) {
  size_t len = sizeof(short) + sizeof(size_t) + filename.length() + 1;
  len = std::max((size_t)10, len); // for reasons, make this at least 10
  char *buf = new char[len]();
  ...
  *(size_t *)(buf + sizeof(short)) = htonll(lines);
  ...
  for (int sock : sockets) {
    ...
    if (!ret) {
      // log("[!!!] [%s] bad return value for client with sock %d", __func__,
      // sock);
      const char *c = __func__;
      log("[!!!] [%s] bad return value for client with sock %d", c, sock);
    }
