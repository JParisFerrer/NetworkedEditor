...
void LockFreeList::insert(size_t line, size_t index, int input) {
  ...
  if (input == ENTER_KEY) {
      ...
      // get new iterators
      this->data[line].erase(this->data[line].begin() + index,
  data[line].end());
  }
  else{
      this->data[line].insert(itHorz+index, input);
  ...
  if (input == ENTER_KEY) {
    ...
    if (!ret) {
      fprintf(stderr, "[!!] Bad line of %lu (max len: %lu) in %s\n", line,
              dataLength * CHARBUFFER, __func__);
      return;
    }
...
void LockFreeList::print(WINDOW *win, size_t line, size_t maxWidth) {
  ...
  //         {
  //             mvwaddch(mainWindow, index, printindex,
  //             t->data.load()[bufindex] [bufoffset]); printindex++;
  //         }
  //     }
  //
  //     t = t->next;
