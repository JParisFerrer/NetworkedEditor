...
void move_win_rel(WINDOW *win, int xoffs, int yoffs) {
  ...
  if (win == mainWindow) {
    ...
    capped_x =
        std::min((ssize_t)(x + xoffs), (ssize_t)text.line_width(capped_y));
...
int network_setup() {
  ...
  int ret = getaddrinfo(SERVER_ADDRESS.c_str(), SERVER_SEARCH_PORT.c_str(),
                        &hints, &server_info);
  if (ret) {
  ...
  for (traverser = server_info; traverser != nullptr;
       traverser = traverser->ai_next) {
    if ((socket_fd = socket(traverser->ai_family, traverser->ai_socktype,
                            traverser->ai_protocol)) == -1) {
      perror("client: socket");
      continue;
    }

    if (setsockopt(socket_fd, IPPROTO_TCP, TCP_NODELAY, (void *)&no,
                   sizeof(int)) == -1) {
      perror("setsockopt");
      return 5;
    }
    ...
    log("Connected to server at %s on port %s", SERVER_ADDRESS.c_str(),
  ...
  if (traverser == nullptr) {
    log("Failed to connect to '%s' using port '%s'", SERVER_ADDRESS.c_str(),
        SERVER_SEARCH_PORT.c_str());
    return 2;
  }
...
void handle_disconnect() {
  int c;
  const char *msg =
      "Disconnected! (s)ave locally and quit, (c)ontinue locally, or (q)uit?";
...
void handleMessage(std::pair<char *, size_t> msg) {
  ...
  // log("Client got message of type %d, %s", (short)type, ((short)type <
  // PacketTypeNum ? PacketTypeNames[(short)type].c_str() : ""));
  ...
  default: {
    log("Client got unhandled message type %d = %s", (short)type,
        ((short)type < PacketTypeNum ? PacketTypeNames[(short)type].c_str()
                                     : ""));
    break;
  }
  }
...
void handleMessages() {
  ...
  while (1) {
    ...
    if (msg.first) {
      ...
      // moved to inside handleMessage so that we can choose not to unlock it
      // (for full reqs)
      // mlock.unlock();
            ...
            if (v[0] == "w") {
              ...
              if (v.size() == 2) {
                /*
                   ...
                   asprintf(&c, "Read from file: %s, %lu lines", v[1].c_str(),
                   newlines); print_in_cmd_window(c); free(c);        // I think
                   it uses malloc
                   */
