...
int setup_network() {
  ...
  for (trav = server_info; trav != nullptr; trav = trav->ai_next) {
    if ((socket_fd = socket(trav->ai_family, trav->ai_socktype,
                            trav->ai_protocol)) == -1) {
    ...
    if (setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) ==
            -1 //||
               // setsockopt(socket_fd, IPPROTO_TCP, TCP_NODELAY, (void*)&yes,
               // sizeof(int)) == -1
...
void forward_message(std::pair<char *, size_t> msg, int ignore) {
  ...
  for (Client &c : clients) {
    if (c.id != ignore && c.alive) {
      // log("Forwarded message from %d to %d (sock: %d)", ignore, c.id,
      // c.socket);
...
void broadcast_message(char *buf, size_t len) {
  ...
  for (Client &c : clients) {
    log("\t id: %d, alive: %s, sock: %d", c.id, (c.alive ? "yes" : " no"),
        c.socket);
    if (c.alive)
...
int server_entrypoint() {
  ...
  while (1) {
    ...
    int yes = 1, no = 0;
    if (setsockopt(client_fd, IPPROTO_TCP, TCP_NODELAY, (void *)&no,
                   sizeof(int)) == -1) {
      perror("setsockopt");
      return 5;
    }
    ...
    pthread_create(&threads.back(), nullptr, thread_routine,
                   (void *)(long)(clients.size() - 1));
  }
