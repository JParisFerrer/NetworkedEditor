...
/*Constructors*/
BlockingVector::BlockingVector() { data.push_back(std::vector<int>()); }
...
void BlockingVector::printColored(WINDOW *win, std::string text) {
  ...
  std::vector<std::pair<std::string, int>> keywords = {
      std::make_pair("\\bfor\\b", 3), std::make_pair("\\bwhile\\b", 3),
      std::make_pair("\\bdo\\b", 3), std::make_pair("[\\d]+", 1),
      std::make_pair("\"[^\"]*\"", 4)};

  std::string types[] = {"int",     "long",  "string", "char",    "size_t",
                         "ssize_t", "short", "bool",   "[\\w]+_t"};
    ...
    try {
      ...
      while (std::regex_search(temptext, m, std::regex(r.first))) {
        ...
        for (int i = 0; i < m.size(); i++) {
          ...
          // fprintf(stderr, "got match '%s' at index %d, pos %ld and len
          // %ld\n", m[i].str().c_str(), i, m.position(i), m.length(i));

          for (int po = m.position(i), le = m.length(i), in = po; in < po + le;
               in++) {
