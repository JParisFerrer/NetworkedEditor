...
static std::string PacketTypeNames[] = {
    "Debug",          "Logging",      "Move",         "Insert",
    "Remove",         "GetFull",      "FullContent",  "WriteToDisk",
    "WriteConfirmed", "ReadFromDisk", "ReadConfirmed"};
...
template <typename T> T get_bytes_as(char *msg, size_t start_byte) {
...
template <class T> bool send_full_content(int sock, TextContainer<T> &text) {
...
template <class T>
bool broadcast_full_content(const std::vector<int> &sockets,
                            TextContainer<T> &text) {
